/**
 * =====================================================================
 * layout.js — キーボードの配列と「どの指で押すか」の対応表
 * =====================================================================
 * 学校の Chromebook を見たまま画面に出すためのデータです。
 *
 * ・日本語配列（JIS）を既定にしています。GIGA スクールの Chromebook は
 *   日本語配列が多く、Enter の形や「かな」「英数」の位置がちがうと、
 *   画面のキーボードを見ても自分の手もとと重ならないためです。
 * ・Windows キーの位置にあるのは Chromebook の**けんさくキー**、
 *   いちばん上の列は**ブラウザの操作キー**です。そこも本物どおりに出します。
 * ・キーの判定には `KeyboardEvent.code`（キーの位置）を使います。
 *   `key`（打たれた文字）は配列や IME でかわりますが、位置は変わりません。
 *
 * 指の色分けはユニバーサルデザインの考え方から、**色だけに頼りません**。
 * どのキーにも指の名前を持たせ、画面には「ひだりの ひとさしゆび」のように
 * ことばでも出します。
 */
(function (global) {
  'use strict';

  /** 指の定義（左から右へ色相がまわるので、色の順＝指の順として覚えられます） */
  const FINGERS = {
  ...
  /** キーの位置（code）→ 指。ここに無いキーは指の色を付けません */
  const FINGER_OF = {};
  ...
  /**
   * キー1つの定義。
   * code: KeyboardEvent.code / lo: そのまま押したときの文字 / up: Shift を足したときの文字
   * kana: 日本語配列のキーに書いてあるかな（配列を見ためどおりにするための表示専用）
   * w: 幅（1 = ふつうのキー） / h: 高さ（2 = JIS の Enter）
   * label: 文字ではないキーの表示名
   */
  const K = (code, lo, up, kana, opt) => Object.assign({ code, lo, up, kana: kana || '', w: 1, h: 1 }, opt || {});

  /** いちばん上の列（Chromebook のブラウザ操作キー）。練習では使わず、見た目をそろえるために出します */
  ...
  /**
   * 打ちたい文字 → そのキーの位置（code）と Shift が必要かどうか。
   * 画面のキーボードで「次に押すキー」を光らせるために使います。
   */
